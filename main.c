 /*
 *  DESCRIPTION: driver file for waiting list implementation
 */
 ...
 while ((ch = getNextNWSChar ()) != EOF)
   {
    ...
    if ('q' == ch)
      ...
      {
       doCallAhead(&waitingList);
      }
    else if('w' == ch)
      {
       doWaiting(waitingList);
      }
    else if('r' == ch)
      {
       doRetrieve(&waitingList);
      }
    else if('l' == ch)
      {
       doList(waitingList);
